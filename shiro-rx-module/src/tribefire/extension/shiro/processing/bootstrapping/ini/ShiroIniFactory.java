...
// ============================================================================
package tribefire.extension.shiro.processing.bootstrapping.ini;
	...
	public String get() {
		...
		for (ShiroClient client : configuration.getClients()) {
			...
			if (client instanceof ShiroOpenIdClient) {
				...
				addConfiguration(authClient, "oidcConfig_" + name + ".includeAccessTokenClaimsInProfile", soidc.getIncludeAccessTokenClaimsInProfile());
			...
			authClient.getFilters().put(name + "_Filter", "org.pac4j.jee.filter.SecurityFilter");
			authClient.getFilters().put(name + "_Filter.config", "$config");
			authClient.getFilters().put(name + "_Filter.clients", name);

			authClient.setUrlPart("/" + loginServletPath + "/auth/" + client.getName().toLowerCase());
		...
		if (StringTools.isBlank(callbackUrl)) {
			throw new IllegalStateException("Callback URL must be configured");
			//callbackUrl = "http://localhost:8080/tribefire-services/" + loginServletPath + "/auth/callback";
		}
