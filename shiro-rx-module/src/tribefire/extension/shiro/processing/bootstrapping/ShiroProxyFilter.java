...
import dev.hiconic.servlet.impl.ProxyAwareHttpServletRequest;
...
public class ShiroProxyFilter implements Filter {
		...
		try {
			...
			if (request instanceof HttpServletRequest) {
				...
				if (pathInfo != null) {
					...
					if (lowerPathInfo.contains(servletPath)) {
						logger.debug(() -> "PathInfo: " + pathInfo + " indicates access to Shiro Module. Wrapping Request.");
						delegated = true;
						ProxyAwareHttpServletRequest wrappedRequest = new ProxyAwareHttpServletRequest(publicServicesUrl, servletRequest);
	...
	private void logTraceFilterInfo() {
	...
	private SecurityManager commonManager() {
		try {
