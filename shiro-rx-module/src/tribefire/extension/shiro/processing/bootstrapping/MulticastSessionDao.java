...
public class MulticastSessionDao extends CachingSessionDAO {
	...
	protected Serializable doCreateInternal(Session session) {
		...
		if (id == null) {
			// Undertow removes everything from the first '@' onward, which leads to an error because the session won't be found:
			// org.pac4j.oidc.exceptions.OidcMissingSessionStateException: State cannot be determined
			id = RandomTools.newStandardUuid() + "-" + instanceIdAsString.replace("@", "-");
